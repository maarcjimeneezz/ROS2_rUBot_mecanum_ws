...
class WallFollower(Node):
    ...
    def laser_callback(self, scan):
        ...
        # If nothing is close â stop
        if closest_value == float('inf'):
            self.cmd = Twist()
            self._state_action = "CLEAR â STOP"
            return

        # ----------- REACT ONLY WHEN BELOW LIMIT -------------
        if closest_value >= self.base_distance:
            # Nothing too close â go forward
            twist.linear.x = self.v_lin
            twist.linear.y = 0.0
            twist.angular.z = 0.0
            action = f"SAFE ({closest_value:.2f}) â FORWARD"
        else:
            # ----------- MIN DISTANCE REACHED â MOVE HOLONOMICALLY ----------
            if closest_sector == "FRONT":
                twist.linear.x = 0.0
                twist.linear.y = +self.v_lin
                action = f"FRONT {closest_value:.2f} â MOVE LEFT"

            elif closest_sector == "FR_RIGHT":
                twist.linear.x = +self.v_lin
                twist.linear.y = +self.v_lin
                action = f"FRONT-RIGHT {closest_value:.2f} â MOVE FRONT-LEFT"

            elif closest_sector == "RIGHT":
                twist.linear.x = +self.v_lin
                twist.linear.y = 0.0
                action = f"RIGHT {closest_value:.2f} â MOVE FORWARD"

            elif closest_sector == "BACK_RIGHT":
                twist.linear.x = +self.v_lin
                twist.linear.y = -self.v_lin
                action = f"BACK-RIGHT {closest_value:.2f} â MOVE FRONT-RIGHT"

            elif closest_sector == "FRONT_LEFT":
                twist.linear.x = -self.v_lin
                twist.linear.y = +self.v_lin
                action = f"FRONT-LEFT {closest_value:.2f} â MOVE BACK-LEFT"

            elif closest_sector == "LEFT":
                twist.linear.x = -self.v_lin
                twist.linear.y = 0.0
                action = f"LEFT {closest_value:.2f} â MOVE BACKWARDS"

            elif closest_sector == "BACK_LEFT":
                twist.linear.x = -self.v_lin
                twist.linear.y = -self.v_lin
                action = f"BACK-LEFT {closest_value:.2f} â MOVE BACK-RIGHT"

            elif closest_sector == "BACK":
                twist.linear.x = 0.0
                twist.linear.y = -self.v_lin
                action = f"BACK {closest_value:.2f} â MOVE RIGHT"
